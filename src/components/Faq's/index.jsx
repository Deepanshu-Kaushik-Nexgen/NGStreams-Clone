        ...
        <>
            <div className="faqMainDiv">
                <div className="headingsDiv">
                    ...
                    <h5>Curious about NGStreams? Let's clarify all your questions!</h5>
                    <h5>Discover our unique approach, project timelines, and what sets us apart in the NGStreams.</h5>
                </div>
                <div className="accordianDiv">
                    <div className="accordian">
                    {Faq.map((item) => {
                        return <>
                            <div className="accordianQues"  onClick={() => toogleAccordian(item.id)} key={item.id}><span className='question'>{item.ques}</span><span><img  className={`${activeButton && activeButton === item.id?"transform180":""}`} src={iconDown} /></span></div>
                            {activeButton === item.id && <div className="answerDiv">{item.ans.map((item) => {
                                return <div className="answer"><p>{item.text}</p></div>
                            })}</div>}
                        </>
                    })}
                    </div>
